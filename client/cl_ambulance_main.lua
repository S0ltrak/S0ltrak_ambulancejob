...
AddEventHandler('esx:onPlayerSpawn', function()
    ...
    if firstSpawn then
        firstSpawn = false
        if ConfigTiscoJobs.ambulance.SaveDeathStatus then
            while not ESX.PlayerLoaded do
                Wait(1000)
            end
            ESX.TriggerServerCallback('ztisco:ambulance:getDeathStatus', function(shouldDie)
                if shouldDie then
                    Wait(1000)
...
CreateThread(function()
    for k, v in pairs(ConfigTiscoJobs.ambulance.Hospitals) do
...
RegisterNetEvent('ztisco:ambulance:clsearch', function(medicId)
    local playerPed = PlayerPedId()
...
function OnPlayerDeath()
    ...
    TriggerServerEvent('ztisco:ambulance:setDeathStatus', true)
    StartDeathTimer()
    StartDeathCam()
    StartDistressSignal()
end

RegisterNetEvent('ztisco:ambulance:useItem', function(itemName)
    ESX.CloseContext()
    ...
    if itemName == 'medikit' then
        ...
        ESX.Streaming.RequestAnimDict(lib, function()
            ...
            TriggerEvent('ztisco:ambulance:heal', 'big', true)
    ...
    elseif itemName == 'bandage' then
        ...
        ESX.Streaming.RequestAnimDict(lib, function()
            ...
            TriggerEvent('ztisco:ambulance:heal', 'small', true)
...
function StartDistressSignal()
    CreateThread(function()
        local timer = ConfigTiscoJobs.ambulance.BleedoutTimer
...
function SendDistressSignal()
    ...
    TriggerServerEvent('ztisco:ambulance:onPlayerDistress')
end
...
function StartDeathTimer()
    ...
    if ConfigTiscoJobs.ambulance.EarlyRespawnFine then
        ESX.TriggerServerCallback('ztisco:ambulance:checkBalance', function(canPay)
            canPayFine = canPay
        end)
    end
  
    local earlySpawnTimer = ESX.Math.Round(ConfigTiscoJobs.ambulance.EarlyRespawnTimer / 1000)
    local bleedoutTimer = ESX.Math.Round(ConfigTiscoJobs.ambulance.BleedoutTimer / 1000)
    ...
    CreateThread(function()
        ...
        while bleedoutTimer > 0 and isDead do
            ...
            if not ConfigTiscoJobs.ambulance.EarlyRespawnFine then
            ...
            elseif ConfigTiscoJobs.ambulance.EarlyRespawnFine and canPayFine then
                text = text .. TranslateCap('respawn_bleedout_fine', ESX.Math.GroupDigits(ConfigTiscoJobs.ambulance.EarlyRespawnFineAmount))
  
                if IsControlPressed(0, 38) and timeHeld > 120 then
                    TriggerServerEvent('ztisco:ambulance:payFine')
                    RemoveItemsAfterRPDeath()
                    break
                end
            end
...
function GetClosestRespawnPoint()
    ...
    for k, v in pairs(ConfigTiscoJobs.ambulance.RespawnPoints) do
...
function RemoveItemsAfterRPDeath()
    TriggerServerEvent('ztisco:ambulance:setDeathStatus', false)
  
    CreateThread(function()
        ESX.TriggerServerCallback('ztisco:ambulance:removeItemsAfterRPDeath', function()
...
RegisterNetEvent('ztisco:ambulance:revive', function()
    local playerPed = PlayerPedId()
    local coords = GetEntityCoords(playerPed)
    TriggerServerEvent('ztisco:ambulance:setDeathStatus', false)
...
-- Load unloaded IPLs
if ConfigTiscoJobs.ambulance.LoadIpl then
...
RegisterNetEvent('ztisco:ambulance:OutVehicle', function()
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped, false)
        TaskLeaveVehicle(ped, vehicle, 16)
        ESX.ShowNotification('🚑 ~g~Vous avez été sorti de votre véhicule')
    else
